...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        for id in iterator:
            ...
            warmup_start_date = start_date - (
                self.cfg.seq_length_hindcast - self.cfg.predict_last_n
            ...
            df_ts = df_ts.loc[
                ...
                list(
                    dict.fromkeys(self.hindcast_input + self.cfg.target + additional_flag)
                ),
            ]
            ...
            df_attr = self.df_attributes.loc[id] if self.cfg.static_input else None
            flag = self._validate_samples(df_ts=df_ts, df_attributes=df_attr, check_NaN=check_NaN)
    ...
    def __getitem__(self, id) -> dict[str, torch.Tensor | np.ndarray | dict[str, torch.Tensor]]:
        ...
        sample["y_obs"] = self.y_obs[basin][
            i + 1 - self.cfg.predict_last_n : i + 1,
            :,
        ]
    ...
    def _validate_samples(self, df_ts: pd.DataFrame, df_attributes: pd.DataFrame, check_NaN: bool) -> np.ndarray:
        ...
        if check_NaN:
            ...
            # -------------------------
            i_start = self.cfg.seq_length_hindcast - self.cfg.predict_last_n
